...
def _read_schema(path: Path | None, table_path: Path) -> pd.DataFrame:
    if path is None:
        if table_path.suffix.lower() not in {".xlsx", ".xlsm", ".xls"}:
            raise ConfigValidationError("schema_path is required when table input is not an XLSX workbook")
        try:
            return pd.read_excel(table_path, sheet_name="schema")
        except ValueError as exc:
            raise ConfigValidationError(
                "schema_path not provided and workbook does not contain a 'schema' sheet"
            ) from exc
    if path.suffix.lower() == ".csv":
        return pd.read_csv(path)
    return pd.read_excel(path)
...
def validate_inputs(config: RunConfig) -> dict[str, Any]:
    ...
    schema_df = _read_schema(schema_path, table_path)
