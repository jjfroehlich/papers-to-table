...
- [x] **T023a** Keep run creation UI-driven in practice by returning a created run immediately, then exposing validating/running/terminal state transitions, actionable failure messaging, and diagnostics/config access while the staged runner works in the background.
