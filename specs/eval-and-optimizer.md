...
The suite replicate score-distribution boxplot shows individual replicate observations alongside its summary marks. Each box has a direct numeric mean label above its highest plotted replicate; native optimizer plots use two decimal places, and the plot expands its upper limit when needed so the labels are not clipped. When the primary metric is bounded `content_correctness` or its `correctness` alias, the y-axis lower bound is zero and the upper bound remains automatic. Other primary metrics retain normal autoscaling. Published percentage-scale derivatives may use one decimal percentage-point precision while preserving the same persisted replicate values.
