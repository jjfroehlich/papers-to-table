# Archived Main-App Plan

Archive status: historical, superseded as normative current source, still informative, partially migrated into [../../plan.md](../../plan.md), [../../architecture/monorepo-layout.md](../../architecture/monorepo-layout.md), [../../architecture/integration.md](../../architecture/integration.md), [../../process/change-policy.md](../../process/change-policy.md), and [../../process/testing-strategy.md](../../process/testing-strategy.md).

Original source path: `specs/plan.md`

This file preserves the pre-unification main-app technical plan in archival form. The legacy content below is preserved verbatim from git history except for this archive header.
...
- structured-output recovery is bounded: `json_schema`, explicit `json_object` fallback when `json_schema` is unsupported, explicit downgrade when the provider rejects regex or grammar constraints for the active request shape, explicit prompt-only JSON fallback when both structured modes are unavailable, one stronger retry, bounded syntactic repair with wrapper stripping and balanced-object extraction, then hard failure for that target
...
**Mitigation:** capability probes, typed validation, bounded `json_schema` to `json_object` negotiation, explicit degraded prompt-only JSON fallback when needed, one stronger retry, minimal syntactic repair, and truthful target-level failure when bounded recovery cannot produce a valid result.
