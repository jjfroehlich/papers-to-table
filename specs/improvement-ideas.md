...
2. Prioritize the A2b typed retrieval-scoring follow-up, targeted C2 recovery, D3 vision acceptance/gating, narrow E2 prompt repair, and F measurement/runtime work over broad model shopping or another unscoped candidate-memory prompt injection.
...
|---|---|---|---|
| **A. Retrieval Quality** | Current prepared retrieval index baseline, [Per-Cell Retrieval Improvements](#per-cell-retrieval-improvements), [Table-Aware Retrieval Units](#table-aware-retrieval-units), and optional lexical/hybrid/embedding retrieval backends. | A0: current baseline with prepared indexes and prompt-header orientation. A2b: narrow typed retrieval-scoring markers without page tokens is the current promising branch and needs replication/full-benchmark. A4: evidence-aware reranking is neutral alone but may be retested only with hard-column retrieval diffs or as a small support layer. A3: do not retest the rejected line-based table units unchanged. | Prepared indexes and safer prompt-header orientation are current main behavior. A2b is the best current A-bundle candidate; future A tests should include retrieval artifact diffs and avoid broad runtime penalties. |
...
| **C. Recovery** | [Uncertainty-Gated Recovery](#uncertainty-gated-recovery). | C0: current baseline. C1: rejected broad current-retrieval rescue. C2: narrower recovery using prepared-index evidence. C3: recovery using schema-conditioned candidate memory only after a B1b-style candidate audit proves useful. | Recovery should be capped and judged by net score gain per added runtime plus recovered-correct versus recovered-wrong cells. |
...
| **Test** | Continue Bundle C after the rejected C1 current-retrieval gate: next tests should use a narrower recovery trigger with prepared-index evidence or A2b retrieval context. Do not depend on the rejected B1 candidate census unless a separate candidate-hit audit succeeds first. Measure recovered-correct cells, recovered-wrong cells, added calls, runtime, missing evidence, anchor validity, and score. |
...
| **Direction** | Improve retrieval/context assembly for individual cells instead of batching cells together. Current main already uses prepared indexes and exposes section/table/figure orientation in prompt passage headers while keeping display text source-preserving. The strongest current retrieval follow-up is A2b typed retrieval-scoring markers without page-number tokens. Evidence-aware reranking is neutral alone and should be retested only with retrieval diffs or as a small support layer. Do not repeat the rejected line-based table-unit or threshold-only score-shape prompt-trimming branches unchanged. |
...
| **Test** | Use `google/gemma-4-e4b` and continue Bundle A by replicating or full-benchmarking A2b first. Retest A4 only with hard-column retrieval diffs or as an A2b support layer. Do not retest the exact A3 table-unit implementation unchanged. |
