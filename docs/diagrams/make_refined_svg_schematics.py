...
def diagram_agents(active: str | None = None) -> str:
    active_label = active or "both"
    s=SVG(
        1440,
        390,
        f"Two agent skills: {active_label} highlighted",
        "Comparison of the local-app and agent-kit skills, with the skill described on this page highlighted.",
    )
    heading(s,"Two agent skills")
    entries = [
    ...
    for x, title, body, outcome, name, color, soft in entries:
        key = title.strip('"')
        highlighted = active is None or key == active
        card_color = color if highlighted else COLORS["line"]
        card_soft = soft if highlighted else COLORS["slate_soft"]
        ink = COLORS["ink"] if highlighted else COLORS["muted"]
        s.rect(x,104,648,220,fill=card_soft,stroke=card_color,sw=1.6,rx=18,shadow=highlighted)
        icon_badge(s,x+62,214,name,card_color,COLORS["surface"],54)
        s.text(x+112,165,title,size=27,weight=700,fill=ink)
        s.text(x+112,207,body,size=16,fill=COLORS["muted"],max_width=470)
        s.text(x+112,270,outcome,size=16,weight=700,fill=card_color,max_width=470)
    return s.finish()


def diagram_agents_neutral() -> str:
    return diagram_agents()


def diagram_agents_agent_kit() -> str:
    return diagram_agents("agent-kit")


def diagram_agents_local_app() -> str:
    return diagram_agents("local-app")
...
FIGURES = [
    ...
    ("04_agent_skills_refined", diagram_agents_neutral),
    ("04_agent_skills_agent_kit_refined", diagram_agents_agent_kit),
    ("04_agent_skills_local_app_refined", diagram_agents_local_app),
]
...
- `04_agent_skills_refined`: neutral comparison of the app-backed and standalone agent workflows.
- `04_agent_skills_agent_kit_refined`: agent-kit page variant with the portable skill highlighted.
- `04_agent_skills_local_app_refined`: local-app page variant with the app-backed skill highlighted.
