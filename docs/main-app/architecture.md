...
6. Retrieval selects relevant document chunks for each eligible target cell, including text, tables, captions, and figure-level chunks.
...
11. Optional figure review by the vision model via LM Studio. This is decided per cell, and done when: figure review is enabled config, evidence-aware triggers indicate likely value (for example weak/unclear/contradictory text evidence, directly relevant figure chunks, or a visual request), and figures are available. A generic text-only figure planner can inspect the cell request, first-pass answer, retrieved snippets, and figure catalog before any vision call. It may skip vision, select up to the configured figure cap, or ask for full-page images when layout or panel counting matters.
...
14. Proposals are filtered by validity/support rules and one final proposal record is persisted per cell, with optional `candidate_answers` and `selection_diagnostics` when selection was considered.
...
- Prompt-only vision responses are repaired for recoverable schema issues such as omitted optional diagnostics or `numeric_value_form: "N/A"`, avoiding retries for defaultable fields.
...
Runtime diagnostics distinguish triggered, attempted, succeeded, failed, and suppressed figure review. Proposal and run diagnostics also record planner outcomes, image source, fallback reason, retry/repair signals, and candidate-selection decisions that block figure evidence from overriding stronger text evidence.
