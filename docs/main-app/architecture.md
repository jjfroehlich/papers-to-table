...
- Prepared retrieval indexes are run artifacts under `retrieval/_indexes/`. They preserve source-grounded chunks and lexical scoring metadata, include a document fingerprint guard, and report whether each cell used an index built fresh, loaded from disk, or reused from memory.
